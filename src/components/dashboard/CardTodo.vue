...
const formatDueLabel = (expectedEndDate) => {
  if (!expectedEndDate) return '날짜 미정'
  const daysLeft = Math.ceil((expectedEndDate.getTime() - Date.now()) / 86400000)
  if (daysLeft < 0) return '기한 초과'
  if (daysLeft === 0) return 'D-day'
  if (daysLeft <= 30) return `D-${daysLeft}`
  const m = expectedEndDate.getMonth() + 1
  const d = expectedEndDate.getDate()
  return `${m}/${d}`
}
  ...
  return visibleItems.value
    ...
    .map((item) => {
      ...
      return {
        ...
        dueLabel: formatDueLabel(expectedEndDate),
    ...
    .slice(0, 5)
    .map(({ expectedEndTime, distanceFromNow, pct, ...item }) => item)
})
